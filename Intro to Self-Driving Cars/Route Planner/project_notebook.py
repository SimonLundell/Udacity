...
def create_gScore(self):
    ...
    # be set to infinity.
    g_scores = {}
    for node in range(len(self.map.roads)):
...
def get_current_node(self):
    ...
    for node in self.openSet:
        temp_dict[node] = self.calculate_fscore(node)
    return min(temp_dict, key=self.fScore.get)
...
def distance(self, node_1, node_2):
    ...
    coords_1 = self.map.intersections[node_1]
    coords_2 = self.map.intersections[node_2]
    dist = math.sqrt((coords_1[0] - coords_2[0])**2 + (coords_1[1] - coords_2[1])**2)
    return dist
...
def get_tentative_gScore(self, current, neighbor):
    ...
    return self.gScore[current] + distance(self, current, neighbor)
...
def heuristic_cost_estimate(self, node):
    ...
    goal = self.map.intersections[self.goal]
    current_node = self.map.intersections[node]
    return math.sqrt((goal[0] - current_node[0])**2 + (goal[1] - current_node[1])**2)
...
def calculate_fscore(self, node):
    ...
    # REMEMBER F = G + H
    return get_gScore(self, node) + heuristic_cost_estimate(self, node)
...
def record_best_path_to(self, current, neighbor):
    ...
    self.fScore[neighbor] = self.fScore[current] + calculate_fscore(self, neighbor)
